...
<head>
<meta charset="UTF-8">
<title>커뮤니티 키워드 검색 게시글 목록 : 오송민</title>
...
<div id="container">
	...
	<div class="right-community">
		<br>
		<div>
			<span class="highlight"><i class="bi bi-search"> </i>검색 결과 : 총 ${paging.boardCount} 건 </span>
		</div>
		...
		<table class="table table-hover">
		  	...
		  	<tbody>
		    	...
		    	<c:if test="${empty communityPagingList }">
					<tr>
						<td colspan="6" class="center">검색 결과가 없습니다</td>
					</tr>
				</c:if>
		  	</tbody>
		</table>
		...
		<div>
	 	   ...
	 	   <c:if test="${loginMember != null}">
	 	   		<a class="btn btn-primary" href="insert_community_page?page=${paging.page }" id="writeBtn" role="button">
		...
		<nav aria-label="pagingNav">
		  	<ul class="pagination">
		        ...
		        <c:otherwise>
		        	<li class="page-item">
		        		<a class="page-link" id="aLinkColor" href="get_community_list_by_keyword?page=${paging.blockPreStartPage}&searchCondition=${searchCondition }&searchKeyword=${searchKeyword }">이전</a>
			    	</li>
		        </c:otherwise>
		    	</c:choose>
			    ...
			    <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i" step="1">
			        <c:choose>
			            ...
			            <c:otherwise>
			            	<li class="page-item">
			                	<a class="page-link" id="aLinkColor" href="get_community_list_by_keyword?page=${i}&searchCondition=${searchCondition }&searchKeyword=${searchKeyword }">${i}</a>
		                	</li>
			            </c:otherwise>
			        </c:choose>
			    </c:forEach>
		    	...
		    	<c:choose>
			        ...
			        <c:otherwise>
			        	<li class="page-item">
			                <a class="page-link" id="aLinkColor" href="get_community_list_by_keyword?page=${paging.blockNextStartPage}&searchCondition=${searchCondition }&searchKeyword=${searchKeyword }">다음</a>
		                </li>
			        </c:otherwise>
			    </c:choose>
		  	</ul>
